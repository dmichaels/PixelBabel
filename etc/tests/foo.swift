...
func adjustShiftTotalDebugData(viewSize: Int, cellSize: Int, cellIncrement: Int, shiftTotal: Int) -> AdjustShiftTotalDebugData {
    ...
    func cellCenterIndexString(_ cellCenter: Double, _ cellSize: Int) -> String {
        ...
        print("[\(cellIndexOffset)] a-[\(cellIndexOffsetRoundedString)]-X [\(cellIndexOffsetRounded)] [\(Double(Int(cellIndexOffset)))] [\(cellIndexOffsetEven)]")
        return "a-\(cellIndexOffsetRoundedString)-x"
        // return "\(cellIndex)#" +
               // "\(cellIndexOffsetEven ? String(Int(cellIndexOffset))+"X" : String(format: "%.8f", cellIndexOffset))"
               // "\(String(format: "%.8f", cellIndexOffsetRounded))"
               // "\(cellIndexOffsetRounded)xxx"
    }
...
func adjustShiftTotalDebugVerbose(viewSize: Int, cellSize: Int, cellIncrement: Int, shiftTotal: Int) {
    ...
    print(
        ...
        " \(String(format: "%.2f", data.cellCenter).lpad(6))" +
        // " \(data.cellCenterIndex.rpad(8))" +
        " \(data.cellCenterIndex)foo" +
    "")
}
